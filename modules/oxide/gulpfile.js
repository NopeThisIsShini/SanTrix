...
gulp.task('copySantrix', function (done) {
  if (fs.existsSync('../santrix/js/santrix/santrix.min.js')) {
    ...
    console.log(chalk.yellow('Run yarn build in the repository root to build a local version of SanTrix'));
    const url = 'https://cdn.jsdelivr.net/npm/santrix@1/santrix.min.js';
    const html = fs.readFileSync('./build/index.html', 'utf8');
    fs.writeFileSync('./build/index.html', html.replace('/santrix/santrix.min.js', url));
    done();
  }
});
...
gulp.task('monitor', function (done) {
  ...
  gulp.watch('./src/**/*').on('change', gulp.series('css', 'buildDemos', 'buildSkinSwitcher', 'copySantrix'));
});
...
gulp.task('watch', gulp.series('build', 'buildDemos', 'buildSkinSwitcher', 'copySantrix', 'monitor'));
