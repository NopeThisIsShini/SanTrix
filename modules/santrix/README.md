# SanTrix - JavaScript Library for Rich Text Editing

## Building SanTrix

See the monorepo root readme file for installation instructions.

Now, build SanTrix by using `grunt`. If you don't have `grunt-cli` installed globally, prefix with `yarn` to execute the local grunt.
...
`grunt`
Lints, compiles, minifies and creates release packages for SanTrix. This will produce the production ready packages.
...
`grunt webpack:core`
Builds the demo js files for the core part of santrix this is required to get the core demos working.

`grunt webpack:plugins`
Builds the demo js files for the plugins part of santrix this is required to get the plugins demos working.

`grunt webpack:themes`
Builds the demo js files for the themes part of santrix this is required to get the themes demos working.
...
Minifies the core, adds the silver theme and adds the table and paste plugin into santrix.min.js.

## Contributing to the SanTrix project

SanTrix is an open source software project and we encourage developers to contribute patches and code to be included in the main package of SanTrix.
