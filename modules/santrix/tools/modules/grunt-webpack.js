    ...
    plugins: [
      ...
      new webpack.WatchIgnorePlugin({
        paths: [
          ...
          /santrix\/modules\/.*\/lib\/.*\.(js|map|d\.ts)$/,
...
const all = (plugins, themes, models) => {
  return [
    create({
      ...
      ...buildEntries(plugins, 'plugins', 'Main.ts', 'js/santrix/'),
      ...buildDemoEntries(models, 'models', 'Demo.ts', 'scratch/demos/'),
      ...buildEntries(models, 'models', 'Main.ts', 'js/santrix/'),
      ...buildDemoEntries(themes, 'themes', 'Demos.ts', 'scratch/demos/'),
      ...buildEntries(themes, 'themes', 'Main.ts', 'js/santrix/'),
    }, '../../tsconfig.demo.json'),
    // Note: This can't use the demo tsconfig as it is the core package
    create({
      'js/santrix/santrix.js': 'src/core/main/ts/api/Main.ts'
    }, '../../tsconfig.json')
  ];
};
