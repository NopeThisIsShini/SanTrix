...
const resourceBase = 'santrix.html-i18n';
...
const createSanTrixJs = (translation, resourceName) =>
  `santrix.Resource.add('${resourceName}',\n${util.inspect(translation)});`
...
const generate = (grunt, srcDir, outputDir, resourcePrefix) => {
  findFiles(srcDir).forEach((file) => {
    ...
    const content = createSanTrixJs(translation, `${resourcePrefix}.${langCode}`);
