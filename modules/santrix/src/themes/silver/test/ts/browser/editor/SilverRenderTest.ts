...
describe('browser.santrix.themes.silver.editor.SilverRenderTest', () => {
  ...
  const getRenderState = (editor: Editor): RenderState => {
    ...
    return {
      container: UiFinder.findIn(container, '.tox.tox-santrix').toOptional(),
      sink: UiFinder.findIn(container, '.tox.tox-santrix-aux').toOptional()
    };
  };
