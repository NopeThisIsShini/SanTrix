...
describe('browser.santrix.themes.silver.editor.header.InlineHeaderTest', () => {
  ...
  const pAssertHeaderWidth = (expectedWidth: string, expectedMaxWidth: string) =>
    Waiter.pTryUntil('Could not verify width', () => {
      ...
      const headerWrapper = SelectorFind.descendant(SugarBody.body(), '.tox-santrix--toolbar-sticky-off').getOrDie();
