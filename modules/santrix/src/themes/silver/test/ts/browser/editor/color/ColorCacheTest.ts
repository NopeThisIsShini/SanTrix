...
import { Menu } from 'santrix/core/api/ui/Ui';
import LocalStorage from 'santrix/core/api/util/LocalStorage';
import * as ColorCache from 'santrix/themes/silver/ui/core/color/ColorCache';

describe('browser.santrix.themes.silver.editor.color.ColorCacheTest', () => {
  afterEach(() => {
    LocalStorage.clear();
  ...
  it('TINY-9184: A cache should use legacy default if it exists', () => {
    LocalStorage.setItem('santrix-custom-colors', JSON.stringify([ '#0F00F0' ]));
    const colors = ColorCache.getCurrentColors('default');
