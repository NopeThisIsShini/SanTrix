...
import { EditorUiApi } from 'santrix/core/api/ui/Ui';
...
const setup = (editor: Editor, setupForTheme: ThemeRenderSetup): RenderInfo => {
  ...
  const renderDialogUi = () => {
    ...
    const sinkSpec = {
      dom: {
        tag: 'div',
        classes: [ 'tox', 'tox-silver-sink', 'tox-santrix-aux' ].concat(deviceClasses),
        attributes: {
          ...I18n.isRtl() ? { dir: 'rtl' } : {}
        }
      },
  ...
  const renderPopupUi = () => {
    ...
    const sinkSpec = {
      dom: {
        tag: 'div',
        classes: [ 'tox', 'tox-silver-sink', 'tox-silver-popup-sink', 'tox-santrix-aux' ].concat(deviceClasses),
        attributes: {
          ...I18n.isRtl() ? { dir: 'rtl' } : {}
        }
      },
  ...
  const renderMainUi = () => {
    ...
    const outerContainer = GuiFactory.build(
      OuterContainer.sketch({
        dom: {
          tag: 'div',
          classes: [ 'tox', 'tox-santrix' ]
            .concat(isInline ? [ 'tox-santrix-inline' ] : [])
            .concat(isToolbarBottom ? [ 'tox-santrix--toolbar-bottom' ] : [])
        ...
        behaviours: Behaviour.derive([
          ...
          Disabling.config({
            disableClass: 'tox-santrix--disabled'
          }),
          Keying.config({
            mode: 'cyclic',
