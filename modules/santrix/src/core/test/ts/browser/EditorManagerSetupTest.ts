...
const storeState = () => {
  ...
  const origSanTrix = (window as any).santrix;
  delete (window as any).santrix;
  ...
  return () => {
    ...
    (window as any).santrix = origSanTrix;
  };
};

describe('browser.santrix.core.EditorManagerSetupTest', () => {
  ...
  it('script baseURL and suffix with script in svg', () => {
    viewBlock.update('<svg><script>!function(){}();</script></svg><script src="http://localhost/nonexistant/santrix.min.js" type="application/javascript"></script>');
    EditorManager.setup();
