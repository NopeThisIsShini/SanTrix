...
import Theme from 'santrix/themes/silver/Theme';

import * as ViewBlock from '../module/test/ViewBlock';

describe('browser.santrix.core.EditorManagerCommandsTest', () => {
  const viewBlock = ViewBlock.bddSetup();

  before(() => {
    Theme();
    EditorManager._setBaseUrl('/project/santrix/js/santrix');
  });
  ...
  Arr.each([
    ...
    it(`mceToggleEditor (${test.label})`, (done) => {
      viewBlock.update('<textarea id="ed_1" class="santrix"></textarea>');
      EditorManager.init({
  ...
  it('mceAddEditor', (done) => {
    viewBlock.update('<textarea id="ed_1" class="santrix"></textarea><textarea id="ed_2" class="santrix"></textarea>');
    EditorManager.init({
