...
import ScriptLoader from 'santrix/core/api/dom/ScriptLoader';

describe('browser.santrix.core.dom.ScriptLoaderTest', () => {
  const testScript = '/project/santrix/src/core/test/assets/js/test.js';
  const nestedScript = '/project/santrix/src/core/test/assets/js/nested.js';
  const invalidScript = '/project/santrix/src/core/test/assets/js/invalid.js';
  let loadedCount = 0;

  before(() => {
    Global.santrix_ = { ScriptLoader: ScriptLoader.ScriptLoader };
  });

  after(() => {
    delete Global.santrix_;
  });
