...
declare const santrix: { Resource: Resource };

describe('browser.santrix.core.ResourceTest', () => {
  const origTiny = Global.santrix;
  ...
  const testScript = (id: string, data: string) => `data:text/javascript,santrix.Resource.add('${id}', '${data}')`;

  const addScript = (id: string, data: string) => {
    santrix.Resource.add(id, data);
  };

  const loadScript = (id: string, url: string): Promise<any> =>
    santrix.Resource.load(id, url);

  const unloadScript = (id: string) => {
    santrix.Resource.unload(id);
  };
  ...
  it('invalid id fails', () => {
    ...
    return assertLoadFailure(load, `Script at URL "data:text/javascript,santrix.Resource.add('invalid-id', 'value.4')" did not call \`santrix.Resource.add('script.4', data)\` within 1 second`);
  });
