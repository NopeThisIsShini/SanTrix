...
describe('browser.santrix.core.EditorRemoveTest', () => {
  ...
  const testRemoveStyles = (editor: Editor, expectedStyle: string) => {
    ...
    EditorManager.init({ selector: '#santrix' });
    assertTextareaDisplayStyle(editor, expectedStyle);
  ...
  it('init editor with no display style', async () => {
    const editor = await McEditor.pFromHtml<Editor>('<textarea id="santrix"></textarea>', settings);
    testRemoveStyles(editor, '');
  });

  it('init editor with display: none', async () => {
    const editor = await McEditor.pFromHtml<Editor>('<textarea id="santrix" style="display: none;"></textarea>', settings);
    testRemoveStyles(editor, 'none');
  });

  it('init editor with display: block', async () => {
    const editor = await McEditor.pFromHtml<Editor>('<textarea id="santrix" style="display: block;"></textarea>', settings);
