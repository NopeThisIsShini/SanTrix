...
import * as Options from 'santrix/core/api/Options';

describe('browser.santrix.core.api.OptionsTest', () => {
  it('getBodyId', () => {
    assert.equal(Options.getBodyId(new Editor('id', {}, EditorManager)), 'santrix', 'Should be default id');
    ...
    assert.equal(Options.getBodyId(new Editor('idc', { body_id: 'ida=a,idb=b' }, EditorManager)), 'santrix', 'Should be default id for idc');
  });
