...
import VisualBlocksPlugin from 'santrix/plugins/visualblocks/Plugin';

describe('browser.santrix.core.EditorCleanupTest', () => {
  before(() => {
    VisualBlocksPlugin();
  });
  ...
  const testCleanup = (comment: string, settings: RawEditorOptions, html: string = '<div></div>', fn: (editor: Editor) => void = Fun.noop) => {
    it(comment, async () => {
      ...
      const editor = await McEditor.pFromHtml<Editor>(html, { base_url: '/project/santrix/js/santrix', ...settings });
  ...
  testCleanup(
    ...
    '<div></div>',
    (editor) => {
      editor.dom.loadCSS('/project/santrix/js/santrix/skins/ui/dark/skin.css');
    });

  // Loaded via StyleSheetLoader
  assertPageLinkPresence('/project/santrix/js/santrix/skins/ui/content/default/content.inline.css', false);
  assertPageLinkPresence('/project/santrix/js/santrix/skins/ui/oxide/skin.css', false);
  // Loaded via DOMUtils as per above
  assertPageLinkPresence('/project/santrix/js/santrix/skins/ui/dark/skin.css', false);
});
