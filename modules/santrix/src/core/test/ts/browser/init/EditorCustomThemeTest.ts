...
import { Theme } from 'santrix/core/api/ThemeManager';
import { SanTrix } from 'santrix/core/api/Santrix';

declare const santrix: SanTrix;

describe('browser.santrix.core.init.EditorCustomThemeTest', () => {
  ...
  context('named synchronous custom theme', () => {
    ...
    const SyncTheme = () => {
      santrix.ThemeManager.add('sync_theme', (editor): Theme => {
        const theme: Theme = {
          renderUI: () => {
            const editorContainer = document.createElement('div');
