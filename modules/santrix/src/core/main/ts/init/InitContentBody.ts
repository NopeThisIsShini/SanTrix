...
import { SanTrix } from '../api/Santrix';
...
const makeStylesheetLoadingPromises = (editor: Editor, css: string[], framedFonts: string[]): Promise<unknown>[] => {
  const { pass: bundledCss, fail: normalCss } = Arr.partition(css, (name) => santrix.Resource.has(name));
  const bundledPromises = bundledCss.map((url) => {
    const css = santrix.Resource.get(url);
