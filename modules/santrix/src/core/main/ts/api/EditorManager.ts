...
declare const window: Window & { santrix?: PreInit; sanTrixPreInit?: PreInit };
...
const EditorManager: EditorManager = {
  ...
  /**
   * Major version of SanTrix build.
   *
  ...
  /**
   * Minor version of SanTrix build.
   *
   ...
   * @type santrix.Editor
   * @example
   * sanTrix.activeEditor.selection.getContent();
   * santrix.EditorManager.activeEditor.selection.getContent();
   */
  activeEditor: null,
  focusedEditor: null,
  ...
  setup() {
    ...
    // If santrix is defined and has a base use that or use the old sanTrixPreInit
    const preInit = window.santrix || window.sanTrixPreInit;
    if (preInit) {
      baseURL = preInit.base || preInit.baseURL;
      suffix = preInit.suffix;
    } else {
      // Get base where the santrix script is located
      ...
      for (let i = 0; i < scripts.length; i++) {
        ...
        // santrix.js santrix.min.js santrix.dev.js
        // santrix.jquery.js santrix.jquery.min.js santrix.jquery.dev.js
        // santrix.full.js santrix.full.min.js santrix.full.dev.js
        const srcScript = src.substring(src.lastIndexOf('/'));
        if (/santrix(\.full|\.jquery|)(\.min|\.dev|)\.js/.test(src)) {
    ...
    /**
     * Base URL where the root directory if SanTrix is located.
     *
     * @property baseURL
     * @type String
     */
    ...
    /**
     * Absolute baseURI for the installation path of SanTrix.
     *
     * @property baseURI
   ...
   * <br /><br />
   * Therefore, when using <code>overrideDefaults</code> with the cloud-based editor, any newly integrated options must be combined with the options in <code>santrix.defaultOptions</code>.
   *
   ...
   * };
   *
   * santrix.overrideDefaults({
   *   ...santrix.defaultOptions,
   *   ...customOptions
   * });
   */
   ...
   * <br /><br />
   * For information on basic usage of <code>init</code>, see: <a href="https://www.santrix.org/docs/santrix/1/basic-setup/">Basic setup</a>.
   *
   * @method init
    ...
    const findTargets = (options: RawEditorOptions): HTMLElement[] => {
      if (Env.browser.isIE() || Env.browser.isEdge()) {
        ErrorReporter.initError(
          'SanTrix does not support the browser you are using. For a list of supported' +
          ' browsers please see: https://www.santrix.org/docs/santrix/1/support/#supportedwebbrowsers'
        );
        return [];
      } else if (isQuirksMode) {
        ErrorReporter.initError(
          'Failed to initialize the editor as the document is not in standards mode. ' +
          'SanTrix requires standards mode.'
        );
        return [];
   ...
   * @return {santrix.Editor/Array} Editor instance or an array of editor instances.
   * @example
   * // Adds an onclick event to an editor by id
   * santrix.get('mytextbox').on('click', (e) => {
   *   ed.windowManager.alert('Hello world!');
   * });
   *
   * // Adds an onclick event to an editor by index
   * santrix.get(0).on('click', (e) => {
   *   ed.windowManager.alert('Hello world!');
   * });
   *
   * // Adds an onclick event to an editor by id (longer version)
   * santrix.EditorManager.get('mytextbox').on('click', (e) => {
   *   ed.windowManager.alert('Hello world!');
   * });
   */
   ...
   *
   * @method add
   * @param {santrix.Editor} editor Editor instance to add to the collection.
   * @return {santrix.Editor} The same instance that got passed in.
   */
  add(editor) {
    const self: EditorManager = this;
   ...
   * @return {santrix.Editor} Editor instance that got created.
   */
   ...
   *
   * @method remove
   * @param {santrix.Editor/String/Object} [selector] CSS selector or editor instance to remove.
   * @return {santrix.Editor} The editor that got passed in will be return if it was found otherwise null.
   */
   ...
   *
   * @method setActive
   * @param {santrix.Editor} editor Editor instance to set as the active instance.
   */
  setActive(editor) {
