 ...
 * Consult the SanTrix API Documentation for more details and examples on how to use this class.
 *
 * @class santrix.dom.Selection
 * @example
 * // Getting the currently selected node for the active editor
 * alert(santrix.activeEditor.selection.getNode().nodeName);
 */
 ...
 * @constructor
 * @method Selection
 * @param {santrix.dom.DOMUtils} dom DOMUtils object reference.
 * @param {Window} win Window to bind the selection object to.
 * @param {santrix.dom.Serializer} serializer DOM serialization class to use for getContent.
 * @param {santrix.Editor} editor Editor instance of the selection.
 */
const EditorSelection = (dom: DOMUtils, win: Window, serializer: DomSerializer, editor: Editor): EditorSelection => {
   ...
   * alert(santrix.activeEditor.selection.getContent({ format: 'text' }));
   */
   ...
   * santrix.activeEditor.selection.setContent('<strong>Some contents</strong>');
   */
   ...
   * const bm = santrix.activeEditor.selection.getBookmark();
   *
   * santrix.activeEditor.setContent(santrix.activeEditor.getContent() + 'Some new content');
   *
   * // Restore the selection bookmark
   * santrix.activeEditor.selection.moveToBookmark(bm);
   */
  ...
  const getRng = (): Range => {
    ...
    try {
      ...
      // IE throws unspecified error here if SanTrix is placed in a frame/iframe
    }
   ...
   * santrix.activeEditor.selection.setNode(santrix.activeEditor.dom.create('img', { src: 'some.gif', title: 'some title' }));
   */
