 ...
 *
 * @example
 * const parser = santrix.html.DomParser({ validate: true }, schema);
 * const rootNode = parser.parse('<h1>content</h1>');
 *
 * @class santrix.html.DomParser
 * @version 3.4
 */
...
const DomParser = (settings: DomParserSettings = {}, schema = Schema()): DomParser => {
   ...
   * @return {santrix.html.Node} Root node containing the tree.
   * @example
   * const rootNode = santrix.html.DomParser({...}).parse('<b>text</b>');
   */
