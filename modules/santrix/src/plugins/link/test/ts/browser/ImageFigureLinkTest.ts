...
import * as LinkPluginUtils from 'santrix/plugins/link/core/Utils';
import Plugin from 'santrix/plugins/link/Plugin';

import { TestLinkUi } from '../module/TestLinkUi';

describe('browser.santrix.plugins.link.ImageFigureLinkTest', () => {
  ...
  it('TBA: Select and link the selection, assert link is present', async () => {
    ...
    editor.setContent(
      '<figure class="image">' +
        '<img src="http://moxiecode.cachefly.net/santrix/v9/images/logo.png" />' +
        '<figcaption>SanTrix</figcaption>' +
      '</figure>'
    );
  ...
  it('TBA: Select and unlink the selection, assert link is not present', async () => {
    ...
    editor.setContent(
      '<figure class="image">' +
        '<a href="http://google.com"><img src="http://moxiecode.cachefly.net/santrix/v9/images/logo.png" /></a>' +
        '<figcaption>SanTrix</figcaption>' +
      '</figure>'
    );
    TinySelections.setCursor(editor, [ 0 ], 0);
