...
import { NotificationApi } from 'santrix/core/api/NotificationManager';
import FullscreenPlugin from 'santrix/plugins/fullscreen/Plugin';
import LinkPlugin from 'santrix/plugins/link/Plugin';

describe('browser.santrix.plugins.fullscreen.FullScreenPluginTest', () => {
  ...
  const assertEditorContainerAndSinkState = (editor: Editor, shouldExist: boolean) => {
    ...
    const sink = UiFinder.findIn(contentContainer, '.tox-silver-sink.tox-santrix-aux').getOrDie();
