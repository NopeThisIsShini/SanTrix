...
describe.skip('browser.santrix.plugins.table.InlineEditorInsideTableTest', () => {
  const setupElement = () => {
    const containerHtml = '<table>' +
      '<tbody>' +
      '<tr>' +
      '<td>' +
      '<div class="santrix" style="border: 1px gray solid">a</div>' +
      '</td>' +
      '</tr>' +
      '</tbody>' +
      '</table>';
    ...
    const editorDiv = SelectorFind.descendant<HTMLDivElement>(container, 'div.santrix').getOrDie('Cannot find SanTrix div');
    return {
      element: editorDiv,
