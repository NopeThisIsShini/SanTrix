...
import { detectSantrixBaseUrl, setupSantrixBaseUrl } from '../loader/Urls';
...
const pFromElement = <T extends EditorType = EditorType>(element: SugarElement<Element>, settings: Record<string, any>): Promise<T> => {
  return new Promise((resolve, reject) => {
    ...
    const run = () => {
      ...
      setupSantrixBaseUrl(santrix, nuSettings);
    ...
    if (!Global.santrix) {
      ...
      loadScript(detectSantrixBaseUrl(settings) + '/santrix.js').get((result) => {
        result.fold(() => reject('Failed to find a global santrix instance'), run);
      });
    } else {
      run();
    }
