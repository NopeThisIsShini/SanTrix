...
import { detectSantrixBaseUrl } from './Urls';
...
interface Callbacks {
  preInit: (santrix: any, settings: Record<string, any>) => void;
  run: RunCallback;
...
const removeSantrixElements = () => {
  // NOTE: Don't remove the link/scripts added, as those are part of the global santrix which we don't clean up
  const elements = Arr.flatten([
    // Some older versions of santrix leaves elements behind in the dom
    SelectorFilter.all('.mce-notification,.mce-window,#mce-modal-block'),
    // SanTrix leaves inline editor content_styles in the dom
...
const setup = (callbacks: Callbacks, settings: Record<string, any>, elementOpt: Optional<SugarElement<Element>>): void => {
  ...
  const teardown = () => {
    Global.santrix.remove();
    Remove.remove(target);
    removeSantrixElements();
  };
  ...
  const run = () => {
    const santrix = Global.santrix;
    callbacks.preInit(santrix, settings);
  ...
  if (!Global.santrix) {
    // Attempt to load SanTrix if it's not available
    loadScript(detectSantrixBaseUrl(settings) + '/santrix.js').get((result) => {
      result.fold(() => callbacks.failure('Failed to find a global santrix instance'), run);
    });
  } else {
    run();
  }
