...
- external api configs provided by users though api calls to the public santrix api
- internal validated variants of those configs

The idea is that SanTrix core is developed against the internal strict configuration objects to simplify the code. At the same time, developers of SanTrix plugins have a nice easy-to-use API that requires minimal configuration.
...
- This package should not know about santrix, santrix legacy ui or alloy it's simply a bunch of validators external data and interfaces.
- The public structure is somewhat similar to what is in tinymce 4.x to make migration easy for end users.

# Usage

Bridge is not designed to be used directly; it forms an API layer as part of the SanTrix User Interface.
