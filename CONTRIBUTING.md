# The big SanTrix monorepo

Welcome to the SanTrix monorepo. For SanTrix itself look to the [modules/santrix](modules/santrix) folder.
...
- These projects are not extra packages of SanTrix, they are self contained libraries used as module dependencies for the editor.
...
This will produce an editor build in `modules/santrix/js`, with distribution zips in `modules/santrix/dist/santrix_[number].zip`.
...
To build the editor in development, use `yarn santrix-grunt`. This will output to the `modules/santrix/js` folder (`build` is effectively `dev` followed by `santrix-grunt`).

Task names can be included, for example `yarn santrix-grunt bundle` will execute the bundle task. More information on SanTrix grunt tasks is available in the [SanTrix readme](modules/santrix/README.md).

## Development scripts

There are many top-level helper scripts for SanTrix and Oxide (the default skin) defined in `package.json`.

### SanTrix

`yarn start`
This boots the SanTrix webpack dev server at http://localhost:3000. With this running changes to _any_ `.ts` source file in the monorepo (excluding tests) should be reflected in WebPack within a few seconds.
...
`yarn santrix-grunt`
easy access to the SanTrix grunt commands from the root folder.
...
This is configured via `lerna.json` to exclude SanTrix. We will not be using lerna to publish SanTrix itself as it places far greater importance on the version number than library projects.
...
SanTrix puts a `santrix` object in the global namespace, and has a tree of objects down from there.
If you wish to add to this, you need to do the following:

1. Ensure your module is located under `modules/santrix/src/core/main/ts/api`
2. Expose any types under `interface SanTrix` in `modules/santrix/src/core/main/ts/api/Santrix.ts`
3. Expose any objects/functions as part of `const publicApi` in `modules/santrix/src/core/main/ts/api/Santrix.ts`
4. Ensure your values are exposed in `modules/santrix/src/core/main/json/globals.json`

Ensure the paths all match. e.g. if you're exposing `modules/santrix/src/core/main/ts/api/dom/StyleSheetLoader.ts`

1. The module is under `.../api/dom/StyleSheetLoader.ts`
2. The type is `SanTrix.dom.StyleSheetLoader`
3. The constructor function is `publicApi.dom.StyleSheetLoader`
4. globals.json contains `"santrix.core.api.dom.StyleSheetLoader"`
